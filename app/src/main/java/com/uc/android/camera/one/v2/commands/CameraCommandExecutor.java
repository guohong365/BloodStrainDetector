...
package com.uc.android.camera.one.v2.commands;
...
public class CameraCommandExecutor implements SafeCloseable {
    private class CommandRunnable implements Runnable {
        ...
        public void run() {
            try {
                Log.v(mLog,"Executing command: " + mCommand + " START");
                mCommand.run();
                Log.v(mLog,"Executing command: " + mCommand + " END");
                ...
                // down.
                Log.v(mLog,"Interrupted while executing command: " + mCommand);
            } catch (CameraAccessException e) {
                // If the camera was closed and the command failed, just return.
                Log.v(mLog,"Unable to connect to camera while executing command: " + mCommand);
            } catch (CameraCaptureSessionClosedException e) {
                // If the session was closed and the command failed, just
                // return.
                Log.v(mLog,"Unable to connect to capture session while executing command: " +
                        mCommand);
            } catch (Exception e) {
                Log.e(mLog,"Exception when executing command: " + mCommand, e);
            }
        }
    }
    ...
    public CameraCommandExecutor(Provider<ExecutorService> threadPoolExecutor) {
        mLog = "CommandExecutor";
