...
public class SingleDeviceStateMachine<TDevice, TKey> implements SingleDeviceCloseListener,
    ...
    private static final String TAG = "DeviceStateM";
    ...
    private void closeRequestWithException(Throwable exception) {
        mOpenDevice = null;
        if (mDeviceRequest != null) {
            Log.w(TAG,"There was a problem closing device: " + mDeviceKey, exception);
    ...
    private void shutdown() {
        ...
        try {
            if (!mIsShutdown) {
                mIsShutdown = true;
                Log.i(TAG,"Shutting down the device lifecycle for: " + mDeviceKey);
                mOpenDevice = null;
                mDeviceState = DeviceState.CLOSED;
                mTargetState = TargetState.CLOSED;
                ...
                Log.w(TAG,"Shutdown was called multiple times!");
            }
        } finally {
            mLock.unlock();
        }
