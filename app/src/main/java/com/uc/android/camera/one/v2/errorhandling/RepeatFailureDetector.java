...
final class RepeatFailureDetector extends com.uc.android.camera.one.v2.core.ResponseListener {
    private final String TAG="RepeatFailureDtctr";
    ...
    public void onFailed(CaptureFailure failure) {
        if (failure.getReason() == CaptureFailure.REASON_ERROR) {
            ...
            Log.e(TAG,String.format("onCaptureFailed() REASON_ERROR:  Consecutive error count = %d x" +
            ...
            if (mConsecutiveErrorCount >= mConsecutiveFailureThreshold) {
                ...
                if (mFailureLevel < mRecoveryStrategies.size()) {
                    Log.e(TAG,String.format("onCaptureFailed() REASON_ERROR:  Repeat failure " +
