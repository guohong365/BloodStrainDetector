...
import com.uc.android.camera.one.v2.camera2proxy.ForwardingImageProxy;
import com.uc.android.camera.one.v2.camera2proxy.ForwardingImageReader;
...
final class LoggingImageReader extends ForwardingImageReader {
    ...
    private final String mLog="LoggingImageReader";
    private final AtomicInteger mNumOpenImages;

    public LoggingImageReader(ImageReaderProxy delegate) {
        super(delegate);
    ...
    private void incrementOpenImageCount() {
        ...
        if (numOpenImages >= getMaxImages()) {
            Log.e(mLog,String.format("Open Image Count (%d) exceeds maximum (%d)!",
