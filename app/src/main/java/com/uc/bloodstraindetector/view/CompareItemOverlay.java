 ...
 * touch events, as {@link com.uc.android.camera.app.CameraAppUI} has a touch listener
 ...
 * this class.
 */
public class CompareItemOverlay extends View
    implements CompareItemStatusListener.CompareItemAreaChangedListener {

    public static final float ZOOM_MIN_RATIO = 1.0f;
    private static final int NUM_ZOOM_LEVELS = 7;
    private static final float MIN_ZOOM = 1f;

    private static final String TAG = "CompareItemOverlay";
    ...
    private OnCompareItemTouchedListener mOnCompareItemTouchedListener;
    ...
    public interface OnCompareItemTouchedListener {
        /**
    ...
    public CompareItemOverlay(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public boolean onTouchEvent(MotionEvent m) {
        ...
        if (mOnCompareItemTouchedListener != null) {
            mOnCompareItemTouchedListener.onItemTouched(m);
        }
        return true;
    }

    /**
     * Set an {@link OnCompareItemTouchedListener} to be executed on any preview
     * touch event.
     */
    public void setOnPreviewTouchedListener(OnCompareItemTouchedListener listener) {
        mOnCompareItemTouchedListener = listener;
    }

    @Override
    public void onItemAreaChanged(RectF previewArea) {
    ...
    private class ZoomProcessor implements ScaleGestureDetector.OnScaleGestureListener {
        private final String TAG = "ZoomProcessor";
