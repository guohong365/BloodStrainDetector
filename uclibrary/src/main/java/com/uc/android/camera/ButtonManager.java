...
import com.uc.android.camera.ui.RadioOptions;
...
public class ButtonManager implements SettingsManager.OnSettingChangedListener {
    ...
    public void onSettingChanged(SettingsManager settingsManager, String key) {
        ...
        if (key.equals(Keys.KEY_FLASH_MODE)) {
            ...
            Log.d(TAG, "onSettingChanged: current index=" + index);
    ...
    /**
     * Returns the appropriate {@link com.uc.android.camera.MultiToggleImageButton}
    ...
    /**
     * Returns the appropriate {@link ImageButton}
     * based on button id.  An IllegalStateException will be throw if the
     * button could not be found in the view hierarchy.
     */
    ...
    public void initializeRulerStyleButton(MultiToggleImageButton button, final ButtonCallback cb, final ButtonCallback preCb, int resIdImages){
        ...
        setPreChangeCallback(button, preCb);
        int state=mSettingsManager.getIndexOfCurrentValue(SettingsManager.SCOPE_GLOBAL, Keys.KEY_CAMERA_RULER_STYLE);
        button.setState( state >= 0 && state < 3 ? state:0, true);
