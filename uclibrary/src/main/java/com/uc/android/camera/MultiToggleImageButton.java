...
import com.uc.R;
import com.uc.android.camera.util.Gusterpolator;
...
public class MultiToggleImageButton extends ImageButton {
    private static final String TAG="MultiToggleImageButton";
    /*
    ...
    private void setStateAnimatedInternal(final int state, final boolean callListener) {
        Log.d(TAG, "setStateAnimatedInternal: state=" + state);
        ...
        if (mState == state || mState == UNSET) {
            Log.d(TAG, "setStateAnimatedInternal: not changed or unset. state=" + mState);
        ...
        Log.d(TAG, "setStateAnimatedInternal: to start animation......");
        new AsyncTask<Integer, Void, Bitmap>() {
            ...
            protected void onPostExecute(Bitmap bitmap) {
                Log.d(TAG, "onPostExecute: ....... state="+ state);
                if (bitmap == null) {
                    setStateInternal(state, callListener);
                } else {
                    ...
                    s.addListener(new AnimatorListenerAdapter() {
                        ...
                        public void onAnimationEnd(Animator animation) {
                            Log.d(TAG, "onAnimationEnd: state=" + state);
                            setStateInternal(state, callListener);
    ...
    private void nextState() {
        Log.d(TAG, "nextState: current=" + mState);
        int state = mState + 1;
        if (state >= mImageIds.length) {
            state = 0;
        }
        Log.d(TAG, "nextState: next=" + state);
        setState(state);
    }
